...
impl Interface {
    ...
    // closure in `socket_egress_one` (`incompatible types: †impl Device + ?Sized`). That is a
    // checker crash, not an undischarged obligation -- the `IpRepr::new` fan-in proof is not
    // refuted, it is unreachable until the ICE is fixed. Flip this back to `trusted(no)` to
    // re-test.
