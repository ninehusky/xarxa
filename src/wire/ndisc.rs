...
impl<'a> Repr<'a> {
    ...
    // obligation again. Two things are still needed before `trusted(no)` here:
    //
    //   * a `requires` over `<T as AsMut<[u8]>>::as_mut_reft(p.buffer)` -- note it must be an
    //     existential (`&mut Packet<T>{v: ..}`), not `&mut Packet<T>[@p]`: `set_msg_type` changes
    //     the `code` index, so an indexed `&mut` fails flux's invariance check;
    //   * `NdiscOptionRepr::emit` and `Packet::payload_mut` need buffer-preserving signatures,
    //     the latter by returning a `Buf` (a returned `&mut [u8]` loses its index,
    //     flux-rs/flux#1714).
